class ScatterPlot {
    ...
    init() {
        ...
        self.xmin = d3.min( self.data, d => d.income );
        self.xmax = d3.max( self.data, d => d.income );

        self.ymin = d3.min( self.data, d => d.price );
        self.ymax = d3.max( self.data, d => d.price );
    }
    ...
    update() {
        ...
        self.xvalue = d => d.income;
        self.yvalue = d => d.price;
    ...
    render() {
        ...
        circles
            .on('mouseover', (e,d) => {
                d3.select('#tooltip')
                    .style('opacity', 1)
                    .html(`<div class="tooltip-label">${d.prefecture}</div>(${d.income}, ${d.price})`);
            })
            .on('mousemove', (e) => {
                const padding = 10;
                d3.select('#tooltip')
